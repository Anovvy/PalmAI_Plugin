...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), '..', 'ui', 'counting_dialog_base.ui'))

# ============================================================================
# --- Implementation of the SAHI Post-processing Manual ---
# ============================================================================
...
class InferenceTask(QgsTask):
    ...
    def run(self):
        try:
            p = self.parameters
            plugin_name = "Palm Oil Counter"

            self.setProgress(5)
            self.setDescription("Loading the detection model...")
            ...
            QgsMessageLog.logMessage(f"The model has been successfully loaded onto the device '{p['device']}'.", plugin_name, Qgis.Success)
            ...
            QgsMessageLog.logMessage("Starting the slicing and inference process on a per-tile basis...", plugin_name, Qgis.Info)
            all_predictions = []
            ...
            with rasterio.open(p['raster_path']) as src:
                ...
                for y in y_steps:
                    for x in x_steps:
                        ...
                        progress_percent = int(5 + 77 * (tile_count / total_tiles)) 
                        self.setProgress(progress_percent)
                        self.setDescription(f"Inference tile {tile_count}/{total_tiles}...")
            ...
            QgsMessageLog.logMessage(f"Total detections collected BEFORE post-processing: {len(all_predictions)}", plugin_name, Qgis.Warning)

            QgsMessageLog.logMessage(f"Running a manual post-process: GreedyNMM with the IOS metric...", plugin_name, Qgis.Info)
            ...
            self.setProgress(95)
            self.setDescription("Saving the final result to a shapefile...")
            ...
            with fiona.open(p['bbox_output_path'], 'w', crs=crs_dict, driver='ESRI Shapefile', schema=schema_poly) as poly_collection, \
                 fiona.open(p['centroid_output_path'], 'w', crs=crs_dict, driver='ESRI Shapefile', schema=schema_point) as point_collection:
            ...
            QgsMessageLog.logMessage(f"FATAL ERROR: {e}\nTraceback:\n{self.error_traceback}", plugin_name, Qgis.Critical)
            return False

    def finished(self, result):
        if not result:
            if self.isCanceled():
                self.iface.messageBar().pushMessage("Info", "The process was cancelled by the user.", level=Qgis.Info, duration=5)
            elif self.exception:
                error_message = f"Process failed: {self.exception}. See the Log Messages Panel for details."
                self.iface.messageBar().pushMessage("Error", error_message, level=Qgis.Critical, duration=15)
            return

        if self.total_detections == 0:
            msg = "The process has completed, but no objects were detected."
            if self.parameters.get('extent_path'):
                msg += "Please check the area of interest (Extent) again."
            self.iface.messageBar().pushMessage("Info", msg, level=Qgis.Warning, duration=8)
            return
            
        success_message = f"Process complete! Found {self.total_detections} object. The results are saved and loaded onto the map."
        ...
        try:
            ...
            # Load layer Bounding Box & Styling
            bbox_layer = self.iface.addVectorLayer(bbox_path, Path(bbox_path).stem, "ogr")
            if bbox_layer:
                # Transparan dengan garis merah tebal 1mm
            ...
            QgsMessageLog.logMessage(f"Unable to load the output layer into QGIS: {e}", "Palm Oil Counter", Qgis.Warning)

# ============================================================================
# MAIN DIALOGUE CLASS
# ============================================================================
class CountingDialog(QDialog, FORM_CLASS):
    def __init__(self, iface, parent=None):
        super(CountingDialog, self).__init__(parent)
        self.setupUi(self)
    ...
    def start_inference_task(self):
        weight_filename = 'yolov8n_counting_1.0.pt'
        weight_path = os.path.join(os.path.dirname(__file__), '..', 'models', weight_filename)

        if not os.path.exists(weight_path):
            self.iface.messageBar().pushMessage("Error", f"File weight '{weight_filename}' Not found in the ‘models’ folder!", level=Qgis.Critical)
            return
        ...
        if "GPU" in self.mComboBox_Device.currentText() and torch.cuda.is_available():
            params['device'] = "cuda:0"
        else:
            params['device'] = "cpu"
            if "GPU" in self.mComboBox_Device.currentText():
                self.iface.messageBar().pushMessage("Warning", "CUDA is not available. The process is running on the CPU.", level=Qgis.Warning, duration=5)

        self.task = InferenceTask(f"Processing {os.path.basename(params['raster_path'])}...", params, self.iface)
