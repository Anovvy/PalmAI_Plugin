# -*- coding: utf-8 -*-
"""DeepLabV3+_Detection_Hybrid-PostProsessing.ipynb
"""
...
MODEL_PATH = '/content/drive/MyDrive/Road Detection_Data_/Model/deeplabv3+_roaddetection_1.pth.tar'
# Path to the .tif image to be used for inference
IMAGE_PATH = '/content/drive/MyDrive/Road Detection_Data_/Data Test/Test.tif'
# Path to save the shapefile output
OUTPUT_SHAPEFILE_PATH = '/content/drive/MyDrive/Road Detection_Data_/Hasil Prediksi/DeepLabV3+/DeepLab_Detection_2.1.shp'
# Path for saving raster results
OUTPUT_RASTER_PATH = '/content/drive/MyDrive/Road Detection_Data_/Hasil Prediksi/DeepLabV3+/DeepLab_Detection_2.1.tif'
# Path for storing the probability map (for tuning)
OUTPUT_PROB_MAP_PATH = '/content/drive/MyDrive/Road Detection_Data_/Hasil Prediksi/DeepLabV3+/prob_map_2.0.npy'
...
MIN_OBJECT_SIZE_PIXELS = 399
CLOSING_ITERATIONS = 50
PROBABILITY_THRESHOLD = 0.4132

# --- PATHFINDING PARAMATERS ---
PATHFINDING_MAX_GAP_PIXELS = 400
PATHFINDING_COST_FACTOR = 15.0
PATHFINDING_MAX_AVG_COST = 0.5537
PATHFINDING_BBOX_PADDING = 40

# --- FINAL FILTER PARAMETERS ---
MIN_ROAD_LENGTH_METERS = 2.0

# =============================================================================
# STEP 3: DEFINITION OF THE MODEL ARCHITECTURE (DeepLabV3+)
# =============================================================================
print("Defining the architecture (via TorchVision, to be loaded in a helper function)...")

# =============================================================================
# STEP 4: CREATING A HELPER FUNCTION
# =============================================================================
print("Defining a helper function...")

def load_model(model_path):
    """Loading the DeepLabV3+ model and its weights."""
    ...
    try:
        ...
        model.eval()
        print("Model DeepLabV3+ loaded successfully.")
    except Exception as e:
    ...
    try:
        with rasterio.open(image_path) as src:
            ...
            for i in tqdm(range(0, len(tiles_to_process), BATCH_SIZE), desc="Processing Batches"):
                ...
                with torch.no_grad():
                    preds_logits = model(batch_tensor)['out']
...
def post_process_and_vectorize(prediction_mask, meta, output_vector_path, output_raster_path=None):
    """
    HYBRID POST-PROCESSING v3 (Smart A* + Simplify)
    """
...
def main_pipeline():
    ...
    try:
        print("--- Loading the DeepLabV3+ model ---")
        ...
        print(f"Probability maps are stored in: {OUTPUT_PROB_MAP_PATH}")
