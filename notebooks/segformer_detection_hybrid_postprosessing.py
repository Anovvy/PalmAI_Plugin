# -*- coding: utf-8 -*-
"""SegFormer_Detection_Hybrid-PostProsessing.ipynb
"""
...
# !pip install rasterio geopandas scikit-image tqdm skan torch torchvision pandas networkx scipy transformers accelerate evaluate "evaluate[image]"
...
# =============================================================================
print("Konfigurasi parameter...")
# The path to your model file on Google Drive
MODEL_PATH = '/content/drive/MyDrive/Road Detection_Data_/Model/segformer_b0_roaddetection_1.pth.tar'
...
OUTPUT_SHAPEFILE_PATH = '/content/drive/MyDrive/Road Detection_Data_/Hasil Prediksi/SegFormer/SegForm_Detection_2.1.shp'
# Path for saving raster results
OUTPUT_RASTER_PATH = '/content/drive/MyDrive/Road Detection_Data_/Hasil Prediksi/SegFormer/SegForm_Detection_2.1.tif'
# Path for storing the probability map (for tuning)
OUTPUT_PROB_MAP_PATH = '/content/drive/MyDrive/Road Detection_Data_/Hasil Prediksi/SegFormer/probmap_2.0.npy'
...
MIN_OBJECT_SIZE_PIXELS = 117
CLOSING_ITERATIONS = 50
PROBABILITY_THRESHOLD = 0.4493

# --- ROUTE FINDING PARAMETERS ---
PATHFINDING_MAX_GAP_PIXELS = 450
PATHFINDING_COST_FACTOR = 15.0
PATHFINDING_MAX_AVG_COST = 0.818
PATHFINDING_BBOX_PADDING = 25

# --- FINAL FILTER PARAMETERS ---
MIN_ROAD_LENGTH_METERS = 2.0

# =============================================================================
# STEP 3: DEFINITION OF THE MODEL ARCHITECTURE (SegFormer)
# =============================================================================
print("Defining the architecture (via Hugging Face; to be loaded in a helper function)...")
MODEL_ID = "nvidia/segformer-b0-finetuned-ade-512-512"
NUM_CLASSES = 2 # (0=background, 1=jalan)
...
def load_model(model_path):
    """Loading the SegFormer model and its weights."""
    print(f"Loading model from: {model_path}")

    config = AutoConfig.from_pretrained(MODEL_ID, num_labels=NUM_CLASSES)
    model = AutoModelForSemanticSegmentation.from_config(config).to(DEVICE)

    try:
        checkpoint = torch.load(model_path, map_location=torch.device(DEVICE))
        model.load_state_dict(checkpoint["state_dict"])
        model.eval() # <<-- PENTING
        print("Model SegFormer loaded successfully.")
    except Exception as e:
...
def inference_tiling(model, image_path):
    """Making inferences about large images using the tiling method."""
        ...
        with rasterio.open(image_path) as src:
            ...
            for i in tqdm(range(0, len(tiles_to_process), BATCH_SIZE), desc="Processing Batches"):
                ...
                for window in batch_windows:
                    tile = src.read(window=window)[:3, :, :]
                    tile = np.transpose(tile, (1, 2, 0))

                    tile = val_transform(image=tile)['image']

                    tile = np.transpose(tile, (2, 0, 1)) 
                    batch_tiles.append(torch.from_numpy(tile).float())
                ...
                with torch.no_grad():
                    ...
                    probs = upsampled_logits.softmax(dim=1)[:, 1, :, :]
...
def post_process_and_vectorize(prediction_mask, meta, output_vector_path, output_raster_path=None):
    """
    HYBRID POST-PROCESSING (Smart A*)
    """
    ...
    for index, row in tqdm(final_branch_data.iterrows(), total=len(final_branch_data), desc="Creating Geometries"):
        try:
    ...
    gdf_filtered = gdf[gdf['length_m'] >= MIN_ROAD_LENGTH_METERS].copy()

    print("Sanitizing geometries and resetting index...")
    final_gdf = gdf_filtered.reset_index(drop=True)

    final_count = len(final_gdf)
    initial_count = len(final_branch_data)
    print(f"Kept {final_count} road segments from {initial_count} initial skeleton branches.")
    ...
    final_gdf.to_file(output_vector_path, driver='ESRI Shapefile')
...
def main_pipeline():
    ...
    try:
        ...
        print("\n--- Getting Started with Post-Processing & Vectorisation (Without Simplify) ---")
        ...
        print("Pipa telah selesai dibangun dengan sukses!")
